...
@WebFluxTest(controllers = ExecutionPreviewController.class)
@Import(PermitAllSecurityTestConfig.class)
class ExecutionPreviewControllerTests {
    ...
    void shouldReturnExecutionPreview() {
        ...
        Mockito.when(providerExecutionSupportService.prepareOpenAi(
                        Mockito.any(),
                        Mockito.any(),
                        Mockito.<java.util.List<GatewayToolDefinition>>any(),
                        Mockito.any()))
                .thenReturn(new PreparedChatExecution<>(
                        ProviderType.OPENAI_DIRECT,
                        selectionResult,
                        OpenAiChatOptions.builder()
                                .model("gpt-4o")
                                .promptCacheKey("xag:pc:1:gpt-4o:prefix")
                                .build()
                ));

        webTestClient.post()
                .uri("/admin/execution/preview")
