...
class OpenAiRealtimeControllerTests {
    ...
    void shouldCreateRealtimeClientSecret() {
        ...
        Mockito.when(gatewayResourceExecutionService.executeLifecycleJson(Mockito.eq(1L), Mockito.eq("sk-gw-test"), Mockito.eq("POST"), Mockito.eq("/v1/realtime/client_secrets"), Mockito.eq("resource-orchestration"), Mockito.any()))
                .thenReturn(response);
