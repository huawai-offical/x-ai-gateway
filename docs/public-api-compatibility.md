# 公开 API Native / Lossless 边界与 SDK 样例

> 当前状态：控制台中的 `能力矩阵`、`Native 命名空间兼容`、`站点档案`、`向量检索排障沙盒` 等入口已下线。本文保留公开协议、native/lossless 边界和 API 事实源；凡提到 Vector Stores、Responses `file_search`、Codex smoke 或 provider catalog，均应理解为“后端/API 暂保留”，不是当前控制台主入口。本文不声明 OpenAI API 全量兼容，也不声明 generic OpenAI-compatible 默认成功。
...
- quick start 接入步骤。
- OpenAI、Claude、Gemini、Ollama、Rerank、Web Search 的协议入口与 native/lossless 边界矩阵。
...
- OpenAI Chat 参数支持证明，覆盖 `response_format`、`tools/tool_choice`、`store/metadata`、`modalities/audio`、`web_search_options` 等关键字段。
...
默认核心 provider 收敛为 OpenAI、Azure OpenAI、Xiaomi MiMo、DeepSeek、Qwen、Moonshot、Volcengine、MiniMax、xAI、Perplexity、Cohere、Jina、Mistral、Anthropic、Gemini、Vertex。Dify、OpenRouter、Together、Fireworks、SiliconFlow 与 generic compatible provider 不进入默认核心承诺；如未来重新接入，只能作为非核心 profile 单独声明范围、凭证、预算和验收边界。
...
- `native_compaction_required`：`POST /v1/responses/compact` 必须走 OpenAI Direct native route；无 native 等价时按 `unsupported` 失败处理。
- `native_input_tokens_required`：`POST /v1/responses/input_tokens` 必须走 OpenAI Direct native route；无 native 等价时按 `unsupported` 失败处理。
- `native_hosted_tool_required`：Responses hosted tool 必须走 OpenAI Direct native route 或原厂 hosted lifecycle；非 native / canonical execution 路径不得用本地结果伪装成功。
- `native_image_edit_required`、`native_image_variation_required`、`native_audio_translation_required`：资源型接口不能跨厂商无损翻译时直接失败。

网关不会通过 header、metadata、opaque marker 或 local fake 把不可对应能力伪装成成功。示例：OpenAI surface 转 Gemini native 的 image edit / image variation / audio translation、OpenAI surface 转 Anthropic 的 file object lifecycle、没有 OpenAI Direct native route 的 Responses compact / input_tokens，以及非 native 路径上的 Responses hosted file_search，均必须失败。

OpenAI Direct preset 使用 `openai-native` / `native-first` 口径：Chat、Responses、Webhooks ingress event persistence、Files、Uploads、Models list/get 与 Realtime client secret 已分批闭环；历史 Realtime WebSocket ingress 只保留归档记录，当前公开 API 不声明 `/v1/realtime` 可用。Conversations local lineage、Vector Stores local lifecycle / file attachment / local chunk ingestion / file content read / local text search / file batch 只属于 gateway-local 支撑能力，用于本项目内部对象生命周期与 RAG 支撑，不等价于 OpenAI official/native lifecycle 成功。Responses hosted file_search 默认不再绑定本地 Vector Store 成功返回；只有 OpenAI Direct native route 或后续明确 native hosted lifecycle 才能成功。`unsupportedFeatures` 会明确列出 Vector Stores real embedding/vector index ingestion/semantic search/hosted file_search_call lifecycle、非核心官方 API out-of-scope、Realtime WebSocket/current calls/WebRTC/SIP/translation/transcription 等边界。OpenAI `/v1/batches`、`/v1/fine_tuning/jobs*`、`DELETE /v1/models/{model}` 的 fine-tuned owner-role delete 语义不属于当前公开功能性服务 API。

Anthropic、Gemini、Vertex 与 Codex 同步按功能性服务 API 与各自 native/profile 边界收紧：
...
- Gemini 只保留 generateContent、embeddings、files 等 Google native 支撑面；audio/image 等资源型入口只有在 Gemini native surface 或 Lossless Translation Matrix 明确允许时才执行，OpenAI surface 到 Gemini native 的 image edit、image variation、audio translation 必须 hard fail。`batchGenerateContent`、tuning、eval-style 和 provider admin API 不纳入兼容面。
...
该示例只表示客户端可用 OpenAI-style 协议入口接入网关；实际成功仍取决于目标 provider 是否具备 native 或 provider-specific native profile，以及请求属性是否可无损翻译。
...
- Legacy 参数映射：deprecated `functions/function_call` 只在可无损表达时转换为 `tools/tool_choice`；provider-specific profile 只有在自身 native contract 和 capability snapshot 允许时才保留原始字段，不以 raw fallback 伪造成功。

公开 OpenAPI 已给 `/v1/chat/completions` 添加 request body schema，展示 `response_format`、`tools`、`tool_choice`、`store`、`metadata`、`web_search_options`、`modalities`、`audio`、`stream_options`、`prediction`、`prompt_cache_key` 与 `safety_identifier` 等关键字段。完整 OpenAI Direct 行为以 conformance matrix 和单元测试为准；provider-specific OpenAI-style profile 仍以 native adapter contract、capability matrix 和 Lossless Translation Matrix 为准。

Advanced JavaScript 示例位于 [chat-advanced-parameters.mjs](sdk-examples/javascript/chat-advanced-parameters.mjs)。示例默认不启用 `web_search_options` 或 `audio`，需要分别设置 `X_AI_GATEWAY_CHAT_WEB_SEARCH=1`、`X_AI_GATEWAY_CHAT_AUDIO=1` 后才会发送对应高级字段，避免普通 smoke 把未声明 native 能力的 provider-specific profile 误判为成功路径。
...
- 当前 `sequence_number` 是 gateway 本地编码顺序，不代表 OpenAI 上游原始 SSE 序号；Realtime WebSocket 历史入口见下一节，当前不作为公开可用能力。

## OpenAI Realtime WebSocket（历史归档 / 当前下线）

`/v1/realtime?model=...` 曾作为 OpenAI-style WebSocket 入口做过本地事件代理基线，但在 REQ-20260524-001 的 native/lossless-only 口径下，当前公开 API 不声明该入口可用。任何 Realtime 能力都必须重新绑定真实 provider native route、真实 WebSocket/WebRTC/SIP 语义和 smoke 证据；不能依赖本地 Live Session 事件、`session.created`、`session.updated` 或 audioBytes 估算来伪造成功。

- 历史实现和测试记录只作为归档证据，不进入当前 public OpenAPI。
- 当前文档不提供 `/v1/realtime` 连接参数或可用性承诺。
- Realtime client secret、WebSocket、calls、WebRTC、SIP、translation/transcription 等能力必须按 provider native/profile 与 Lossless Translation Matrix 重新验收。

当前边界：如果客户端请求当前未声明的 Realtime 能力，应得到明确 unsupported/native route required 类错误，而不是本地事件代理成功。
...
`GET /v1/chat/completions` 与 `GET /v1/chat/completions/{completionId}/messages` 使用 OpenAI-style list envelope：`object: "list"`、`data`、`has_more`，非空页会返回 `first_id` 与 `last_id`。分页参数为 `after`、`limit` 和 `order`；`limit` 默认 20，合法范围 1 到 100；`order` 仅支持 `asc` 或 `desc`，默认 `asc`。Chat Completion list 额外支持 `model` 与 `metadata[key]` 过滤。
...
- `input_items` 从创建 Response 时的原始 `input` 生成 OpenAI-style list envelope，支持 `after`、`limit`、`order`，默认 `limit=20`、`order=desc`，`limit` 范围为 1 到 100。
...
- `POST /v1/responses/compact` 在 OpenAI Direct native route 可用时优先转发到上游并保留上游 HTTP 状态；route 不可用、不是 OpenAI Direct native route 或目标上游无 native 等价能力时返回 OpenAI-style `unsupported`，错误码为 `native_compaction_required`。网关不再返回本地 opaque compaction marker，因为本地无法产生 OpenAI 官方 encrypted compaction state。
- 本地 lifecycle 继续按 Distributed Key 隔离，不能读取、删除或取消其他 key 的 Response。

## OpenAI Conversations 生命周期

Conversations 采用 gateway-local lineage，面向 OpenAI-style 客户端提供本地对象生命周期；该能力只作为网关本地支撑能力，不声明 OpenAI official/native Conversations lifecycle 成功：
...
- `POST /v1/conversations/{conversationId}/items`：一次最多追加 20 条 item，返回 OpenAI-style list envelope。
...
控制台中的向量页面和排障沙盒已下线；以下内容仅描述仍暂时保留的公开 `/v1/vector_stores*` gateway-local 支撑面。Responses hosted `file_search` 已按 `TASK-20260524-001-04` 收紧为 native-required，不再通过本地 Vector Store 绑定返回官方成功对象；非 OpenAI Direct/native hosted lifecycle 路径返回 `native_hosted_tool_required`。

Vector Stores 先建立 gateway-local lifecycle 基线，面向 OpenAI-style 客户端提供本地对象生命周期；该能力只作为网关本地 RAG/file_search 支撑能力，不声明 OpenAI official/native Vector Stores 托管语义：

- `POST /v1/vector_stores`：创建 `vs_...` 本地 vector store，支持 optional `name`、`metadata`、`file_ids`、`expires_after` 与 `expires_at`。
- `GET /v1/vector_stores`：返回 OpenAI-style list envelope，支持 `after`、`limit`、`order`，默认 `limit=20`、`order=desc`，`limit` 范围 1 到 100。
...
- 范围：公开 docs、OpenAI-style Chat/Responses/Conversations/Vector Stores/Webhooks、Web Search、Claude Messages、Gemini/Vertex generateContent 标准区、Codex Responses smoke 边界、Video/Music async task、Media provider matrix。
...
## 主流 API Native/Profile 边界说明

- xAI/Grok provider-specific OpenAI-style profile：`/v1/responses` 只在 xAI native/profile 能力允许时保留原始 Responses 字段；Native runtime 已下发 `service_tier`、`parallel_tool_calls`、`prompt_cache_key`、`top_logprobs`、`safety_identifier`、`verbosity` 与 metadata。Responses-only 字段如 `truncation`、`text`、`prompt_cache_retention`、`include`、`previous_response_id` 会进入 provider extra body；Grok prompt cache affinity 使用 `prompt_cache_key` 派生 `x-grok-conv-id`。
- OpenAI Direct 非流式 Responses create 优先返回上游原始 Responses JSON，并将 `model` 重写为 public model；OpenAI Direct `stream=true` 会透明转发上游原始 SSE 事件，保留 upstream event name、data、sequence 与未知字段；只有明确声明为 gateway-local 支撑能力的路径才使用 canonical Responses encoder，目标 provider native/profile 不具备能力或属性不可无损表达时必须 hard fail。
...
- `native_compaction_required`：Responses compact 无 OpenAI Direct native route 或目标上游无 native 等价能力。
- `insufficient_balance`：用户余额或订阅额度不足。

## 当前取舍

本轮提供后端 docs bundle、最小 OpenAPI JSON、本地 Markdown 与主流 API native/profile 边界说明，先让公开 native/lossless 边界可访问、可测试、可翻译，并把“OpenAI-style 协议入口 / provider-specific profile 声明”和“provider-native 能力”明确拆开。完整 OpenAPI 生成器、前端语言切换组件、真实 provider smoke 和第三方 SDK 全量适配留到后续。
