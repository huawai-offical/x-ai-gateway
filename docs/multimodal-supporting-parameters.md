...
| `/v1/audio/transcriptions` | multipart | `file`、`model`、`language`、`prompt`、`response_format`、`temperature`、`timestamp_granularities[]`、`include[]`、`stream` | multipart 字段按文本透传；Gemini native 仅支持已实现 transcription 文本路径。 |
| `/v1/audio/translations` | multipart | `file`、`model`、`prompt`、`response_format`、`temperature` | OpenAI-style 站点直连；Gemini/Vertex/Anthropic 原生互转未完成前不标记为 native。 |
...
| `/v1/images/variations` | multipart | `image`、`model`、`n`、`response_format`、`size`、`user` | OpenAI-style 站点直连；默认模型为 `dall-e-2`。 |
...
- provider capability view 暴露 `audio_translation`、`image_edit`、`image_variation`，但 Gemini/Vertex/Anthropic 原生互转未完成前不误标为 native。
