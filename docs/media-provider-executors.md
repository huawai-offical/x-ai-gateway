...
- 新增 provider-specific media adapter：
  ...
  - `GET /api/v1/music/{musicId}/download` 为 Music 产物下载引用预留统一 API。
...
| Music | Suno-like | `ADAPTER_REQUIRED` | `provider_specific_adapter_required` | 需要签名、轮询和产物映射适配。 |
...
- Suno、MiniMax、Kling、Vidu、Hailuo、Jimeng、Midjourney-like 专有 API adapter 仍需后续推进。
