# TASK-20260514-023 OpenAI Vector Stores 对话 RAG 支撑面
...
官方 Vector Stores 是 Responses `file_search` 和长上下文检索的关键资源族。根据 [REQ-20260518-005](../../docs/requirements/REQ-20260518-005-functional-service-api-scope.md)，本任务不再追求 OpenAI Vector Stores 官方全量 parity，只保留对话/tools/RAG 所需的本地支撑面。

## 目标

- 覆盖对话 RAG 所需的 vector stores create/list/get/update/delete/search。
- 覆盖 vector store files create/list/get/delete/content。
- 保留 Vector Store File Batches 作为 file_search ingestion 支撑，明确它不等同于 OpenAI `/v1/batches`。
...
- 不追求 hosted `file_search_call` 与 OpenAI 官方 Vector Stores 全量 lifecycle parity。
...
- Vector Stores 对话/RAG 支撑路径有 supported/partial/out-of-scope 状态。
...
- 已完成：[TASK-20260518-004 OpenAI Vector Store 本地 Ingestion 产物基线](../done/TASK-20260518-004-openai-vector-store-local-ingestion-artifact-baseline.md)。已在 attachment 创建时固化本地 chunk ingestion metadata，记录 `local-chunk-v1`、真实 usage bytes、content hash、estimated tokens、chunk_count 与 chunks，并让 search 优先复用 ingestion chunks；旧 attachment 保留原文读取回退。
- 剩余切片：真实 embedding/vector index ingestion、语义向量检索、真实 smoke 均不再阻塞当前功能性服务 API 本地支撑面。真实 smoke 统一交由 [TASK-20260514-031](../backlog/TASK-20260514-031-openai-real-smoke-certification-harness.md)，hosted `file_search_call` lifecycle 不再作为当前产品范围必做项。

## 当前状态

- 2026-05-21：Vector Stores 本地 lifecycle、files、file batches、content、search、Responses file_search 绑定和本地 ingestion 产物均已闭环；本任务归档至 `tasks/done/`。

## 2026-05-21 口径补充

- 本任务保留的是 Responses `file_search` / 对话 RAG 所需的后端支撑面。
- 它不代表继续保留独立 `向量 API` 控制台、向量检索排障沙盒或单独向量产品线；这些控制台能力已进入下线范围。
