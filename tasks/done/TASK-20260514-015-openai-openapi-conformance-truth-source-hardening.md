...
当前 `docs/openapi/public-openapi.json` 曾只公开 Chat/Responses/Web Search/Messages/Gemini/Media 等少数路径，但代码已经实现 Embeddings、Audio、Images、Files、Uploads、Moderations、Models、Realtime client secrets 等更多路径。2026-05-18 范围收窄后，OpenAI `/v1/batches`、Fine-tuning events/checkpoints、Models delete 等非核心或 owner-role 官方语义不再属于公开功能性服务 API。

## 目标

- 让 public OpenAPI、provider catalog、public API compatibility docs、endpoint conformance matrix 使用同一个 OpenAI 兼容性事实口径。
- 把已实现且属于对话/tools 支撑面的路径补到公开 OpenAPI，或明确标注为内部/未公开。
- 把非核心 OpenAI 官方资源族写入 `unsupportedFeatures`、accepted exceptions 或 out-of-scope 决策。
