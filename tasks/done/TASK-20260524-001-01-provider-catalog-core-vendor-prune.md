...
当前 `provider-catalog.json` 包含 Dify、OpenRouter、Together、Fireworks、SiliconFlow 等非核心或聚合型 provider。用户要求删除很多无意义厂商，保留 xAI 以及拥有自有模型的头部厂商。

## 目标

- 审计现有 provider preset，按核心保留、候选保留、默认清理分类。
- 从默认核心导入范围移除 Dify 等非自有模型编排平台。
