...
- `src/main/java/com/prodigalgal/xaigateway/admin/application/FunctionalProviderSmokeRecordReplayFixtureVerifier.java`
- `src/main/java/com/prodigalgal/xaigateway/admin/application/FunctionalProviderSmokeCertificationService.java`
...
- coverage matrix provider boundary tests。
- functional provider smoke certification / verifier tests。
...
- 2026-05-24：已完成 SDK 示例、functional service coverage matrix 与 smoke harness 范围收敛。文档明确 provider-specific OpenAI-compatible 不是 generic fallback；Dify、OpenRouter、Together、Fireworks、SiliconFlow 与 generic compatible 不进入默认核心支持和 official smoke。
- 2026-05-24：已收紧 functional provider record/replay fixture 与 verifier。离线 fixture 只允许 `GEMINI_DIRECT` 与 `XIAOMI_MIMO`，MiMo smoke 必须使用 `XIAOMI_MIMO_OPENAI_COMPATIBLE` / `XIAOMI_MIMO_ANTHROPIC_COMPATIBLE` provider-specific protocol，不允许顶层 `OPENAI_COMPATIBLE` 或旧长尾 provider 冒充核心 provider。
- 2026-05-24：已将真实 `FunctionalProviderSmokeCertificationService` 输出与 verifier 统一，子 fixture 会继承父级 smoke provider/protocol/baseUrl 语境，requestPreview 也写入 provider-specific provider/protocol；MiMo OpenAI-compatible smoke 使用 `api-key` 头，非 MiMo 的 `anthropic_compatible` alias 不会被误归一为 MiMo。
...
- 2026-05-24：本任务已完成并准备归档到 `tasks/done/`。父任务剩余工作继续由 `001-02`、`001-03`、`001-04` 以及 backlog `001-06`、`001-07`、`001-08` 承接。
