# TASK-20260507-003 Codex 账号分组热切换、负载均衡与失败恢复 UI
...
`cli_proxy-master` 支持 active config 热切换、模型路由、weight-based 负载均衡、失败阈值、自动恢复和手动 reset。当前项目有账号分组、route policy runtime、账号健康状态和治理页，但 Codex 用户无法在一个页面看到“当前 Codex 正在用哪个账号分组、如何热切换、哪些账号被隔离、如何恢复”。

## 目标

- 为 Codex 账号分组提供热切换与故障恢复操作面。
...
- 前端新增 Codex 账号分组视图或在 account group detail 中增加 `Codex Runtime` tab。
...
- 管理员可在 UI 内完成 Codex 账号分组切换和失败恢复。
...
- 在账号分组详情页新增 Codex Runtime 面板，展示候选账号、冻结/冷却/失败计数、最近错误、quota、成功率和最近使用。
...
- 2026-05-07：进入第四批最高优先级任务闭环，开始实现 Codex 账号分组运营恢复面。
- 2026-05-07：账号分组详情新增 Codex Runtime 面板，展示候选、可路由、冻结、失败、quota、成功率、缓存命中和最近错误。
...
- `bun run test -- src/features/accounts/codex-onboarding-page.test.tsx src/features/accounts/account-group-detail-page.test.tsx src/features/portal/portal-home-page.test.tsx`
- `bun run typecheck`
- `bun run build`
...
- 更新 `web/src/features/accounts/account-group-detail-page.tsx`。
- 更新 `web/src/features/accounts/account-group-detail-page.test.tsx`。
...
- 本任务对应的是旧控制台中的账号分组运行态/恢复 UI。
- 当前保留它是为了记录历史实现与验证证据，不再表示该运行态页面仍是现役产品面。
